...
def get_existed_skills(skill_file_path):
    skill_files = [f for f in os.listdir(skill_file_path) if os.path.isfile(os.path.join(skill_file_path, f))]
    skills = {}

    for skill_file in skill_files:
        try:
            ids = skill_file.replace(".txt", "")
            with open(os.path.join(skill_file_path, skill_file), "r", encoding='utf-8') as f:
                skill_text = f.read()
                skill_dict = json.loads(skill_text)
                skills[ids] = skill_dict
