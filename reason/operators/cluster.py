...
from utils.prompt import group_column_demo
...
def group_column_func(sample, table_info, llm, llm_options=None, debug=False, skip_op=[]):
    ...
    prompt += group_column_build_prompt(table_text, statement, num_rows=5)
    ...
    group_param_and_conf_list = sorted(group_param_and_conf.items(), key=lambda x: x[1], reverse=True)

    thought = responses[0][0].split("Answer:")[0].strip() if "Answer:" in responses[0][0] else responses[0][0].strip() + "\n"

    operator = {
        "operator_name": "group_column",
...
def group_column_act(table_info, operator, strategy="top", skip_op=[]):
    ...
    if len(operator["parameter_and_conf"]) == 0:
        return failure_table_info
    if strategy == "top":
        group_column, group_info = eval(operator["parameter_and_conf"][0][0])
    else:
