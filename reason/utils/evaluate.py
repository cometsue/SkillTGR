...
def wikitq_match_func(sample, target_values, strategy="top"):
    # results = sample["results"]["answer"]
    res = sample["results"]["answer"]
    ...
    pred_answer = [res.lower()] if '|' not in res else [r.strip() for r in res.lower().split('|')]
...
def wikitq_match_func_for_samples(dataset_name, dataset, all_samples, strategy="top", tagged_dataset_path='../data/wikitq/tagged_data'):
    ...
    if dataset_name == "wikitq":
        ...
        for _, item in enumerate(dataset):
            item_id = item['ids']
            item_ans = item['answer'] # list
            target_values_map[item_id] = to_value_list(item_ans)
    ...
    for idx, sample in enumerate(all_samples):
        try:
            if wikitq_match_func(sample, target_values_map[sample['ids']], strategy):
                samples_dict[sample['ids']] = 1
                correct_list.append(1)
            else:
                samples_dict[sample['ids']] = 0
                correct_list.append(0)
        except:
            error_id = 'nu-{}'.format(idx)
            # print("Error id:{}".format(error_id))
            samples_dict[error_id] = 0
            # print(f"Error")
            continue

    total_samples = 4344 if dataset_name == "wikitq" else 2024
    print("correct samples: {}; feasible samples: {}; all samples: {}".format(sum(correct_list), len(correct_list), total_samples))
    # print(f"correct samples: {sum(correct_list)}", f";feasible samples: {len(correct_list)}", f";all samples: {4344}")
    if len(correct_list) > 0:
        print('correct / feasible: {}; '.format(round(sum(correct_list) / len(correct_list), 4)))
    else:
        print('None samples predict correct!!!')
    return round(sum(correct_list) / total_samples, 4), samples_dict
