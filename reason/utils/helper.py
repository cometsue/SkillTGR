...
def triple2table(triples, num_rows=100):
    row_dict = {}
    columns = []
    for rowid, col, cell in triples:

        if rowid not in row_dict:
            row_dict[rowid] = {}
        row_dict[rowid][col] = cell
    ...
    output_lines = []
    output_lines.append("column : " + " | ".join(columns))
    for rowid, cells in row_dict.items():
        display_row = rowid.replace("row", "row ")

        row_values = [str(cells[col]) for col in columns]
        row_line = f"{display_row} : {' | '.join(row_values)}"
        output_lines.append(row_line)
        if len(output_lines) == num_rows+1:
            break
